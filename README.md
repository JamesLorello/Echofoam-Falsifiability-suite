...
### Held-out relaxing internal-variable comparison

The next comparator uses a conventional one-mode Maxwell vector state that
relaxes toward the present amplitude gradient. It shares field and transport
parameters with the kernel, and selects relaxation rate and equilibrium gain
on a separate training seed set. Held-out evaluation includes all four
preparation families plus a diagonal pattern excluded from fitting. Targets
are still generated by the Echofoam kernel, so this remains a synthetic
model-class comparison. See the
[frozen protocol](docs/STANDARD_MEMORY_COMPARISON_PROTOCOL.md).
